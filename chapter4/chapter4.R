...
set.seed(1) # what is this set seed number ? doesn't matter much. it will reset once called
library(caret)
...
trainingRows <- createDataPartition(classes, p = .80, list= FALSE)#why p=0.80 ?
head(trainingRows)
nrow(trainingRows)
...
## createFolds (for k-old cross-validation) and createMultiFolds (for repeated
...
## Fit a 5-nearest neighbor classificationmodel (Fig. 4.3) to the training data 
...
## The knn3 function can produce class predictions as well as
...
## need package kernlab, can not install this package
